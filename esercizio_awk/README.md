# Utilizzo di AWK e grep + sed per manipolare file CSV

## Utilizzo di AWK

Con AWK è possibile manipolare dati di tipo testuale in forma di file o come flusso dati proveniente dallo standard input. Se allo script AWK si passano uno o più file, questi verranno letti riga per riga.
...
- `-F` indica il separatore di campi; `","` sto indicando che utilizzo la virgola come separatore;
...
### Caso in cui il file .csv contiene almeno una riga con esattamente 3 campi:

```bash
grep "banana" file_esempio.csv | sed 's/.*,//'
```

Con `grep "banana"` si considerano le righe del file che contengono solo la stringa banana. Attraverso la pipe `|` l'output del comando di sinistra viene passato come input del comando a destra.
...
### Caso in cui il file .csv contiene almeno una riga con almeno tre campi.

In questo caso, il comando visto nel caso precedente stamperà l'ultimo campo della riga che potrebbe non essere il terzo. Si può dare il comando
...
- `[^,]*` indico qualsiasi carattere che non sia una virgola (`^` dentro `[ ]` indica una negazione); `[^,]*` indica qualsiasi carattere che non sia una virgola che appare zero o più volte, quindi indica il primo campo;
